...
def read_pressure(ed):
    ...
    print(f"Raw pressure response: {current_pressure}")  # Print the raw response

    if len(current_pressure) > 0:
        current_pressure = str(current_pressure[0])  # Convert bytes to string
        print(f"String converted response: {current_pressure}")  # Check string format

        current_pressure = float(current_pressure[2:-3])  # Extract the numeric part
        print(f"Extracted float value: {current_pressure}")  # Final value
    else:
        time.sleep(0.5)
        ED.write(str.encode("?GA1\r"))  # Retry
        current_pressure = ED.readlines()

        print(f"Retry response: {current_pressure}")  # Print retry response
